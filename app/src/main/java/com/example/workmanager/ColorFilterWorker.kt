    ...
    override suspend fun doWork(): Result {
        ...
        return imageFile?.let { file ->
            ...
            withContext(Dispatchers.IO) {
                val resultImgFail = File(context.cacheDir, "new-image.jpg")
                val outputStream = FileOutputStream(resultImgFail)
                ...
                if(successful) {
                    Result.success(
                        workDataOf(
                            WorkerKeys.FILTER_URI to resultImgFail.toUri().toString()
                        )
                    )
                } else Result.failure()
            }
